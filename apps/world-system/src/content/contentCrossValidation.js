...
function validateNpcShops(npcShops, itemIds, errors) {
  const rows = toArray(npcShops);
  for (let i = 0; i < rows.length; i += 1) {
    const shop = rows[i];
    if (!isObject(shop)) continue;
    const vendorId = String(shop.vendor_id || "").trim();
    const stockItems = toArray(shop.stock_items);

    for (let j = 0; j < stockItems.length; j += 1) {
      const itemId = String(stockItems[j] || "").trim();
      if (itemId && !itemIds.has(itemId)) {
        addError(errors, {
          content_type: "npc_shop",
          entry_id: vendorId || null,
          field: "stock_items",
          missing_reference: itemId,
          message: "npc shop stock item does not exist in items content"
        });
      }
    }
  }
...
function validateCrossContentReferences(content) {
  ...
  const npcShops = toArray(data.npc_shops);
  ...
  validateNpcShops(npcShops, itemIds, errors);
