...
function finalizeCombatMutation(input) {
  ...
  const progression = progressCombatAfterResolvedTurn({
    ...
    confusion_direction_rng: context.confusionDirectionRng,
...
function processCombatCastSpellRequest(input) {
  ...
  const out = performCastSpellAction({
    ...
    command_word: payload.command_word || null,
    damage_type: payload.damage_type || null,
    hazard_side: payload.hazard_side || null,
    ...
    hazard_area_tiles: Array.isArray(payload.hazard_area_tiles) ? payload.hazard_area_tiles : null,
    ...
    targeting_save_fn: context.targetingSaveFn,
    targeting_save_bonus_rng: context.targetingSaveBonusRng,
    spellcast_check_fn: context.spellcastCheckFn,
