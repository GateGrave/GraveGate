...
async function main() {
  ...
  const enemiesRoot = path.resolve(repoRoot, "apps/map-system/assets/tokens/enemies");
  const processedRoot = path.resolve(enemiesRoot, "processed");
  const catalogPath = path.resolve(repoRoot, "apps/map-system/data/tokens/enemy-token-catalog.json");

  fs.mkdirSync(processedRoot, { recursive: true });

  const rawFiles = fs.readdirSync(enemiesRoot)
  ...
  for (const fileName of rawFiles) {
    const inputPath = path.join(enemiesRoot, fileName);
    ...
    existingById.set(tokenChoiceId, {
      ...
      notes: previous.notes || "Batch-processed enemy token."
    });
